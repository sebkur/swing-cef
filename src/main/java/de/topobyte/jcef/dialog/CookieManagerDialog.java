...
public class CookieManagerDialog extends JDialog
{
			...
			implements CefCookieVisitor
	{
		...
		{
			super();
			columnNames = new String[] { "Name", "Value", "Domain", "Path",
					"Secure", "HTTP only", "Created", "Last Access",
					"Expires" };
		}

		// add an entry to the table
		@Override
		public boolean visit(CefCookie cookie, int count, int total,
				BoolRef delete)
		{
			Object[] entry = { cookie.name, cookie.value, cookie.domain,
					cookie.path, Boolean.valueOf(cookie.secure),
					Boolean.valueOf(cookie.httponly), cookie.creation,
